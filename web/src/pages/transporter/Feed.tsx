  ...
  return (
    <Layout>
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', top: -60, right: -60, width: 200, height: 200, borderRadius: '50%', background: 'radial-gradient(circle, rgba(96,165,250,0.1) 0%, transparent 70%)', pointerEvents: 'none' }} />
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
          <div>
            <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#4ADE80', marginBottom: 8 }}>Transporter Dashboard</p>
            <h1 style={{ fontSize: '1.7rem', fontWeight: 800, color: '#E8F0EB', letterSpacing: '-0.02em' }}>Delivery Requests</h1>
            <p style={{ fontSize: 14, color: '#4A6B58', marginTop: 6 }}>Open requests in your region — accept to claim</p>
          </div>
          <span style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16, marginBottom: 24 }}>
        ...
        <MetricCard label="Active Deliveries" value={active.length} sub="Accepted or in transit" trend={active.length > 0 ? `${active.length} active` : undefined} icon={<TruckMetricIcon />} accent="#C9A84C" />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        ...
        <div className="card" style={{ padding: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <h2 style={{ fontWeight: 700, fontSize: 15, color: '#111827' }}>Open Requests</h2>
            <button onClick={fetchData} style={{ fontSize: 12, padding: '6px 14px', borderRadius: 9999, border: '1px solid #D1E0D8', background: 'transparent', color: '#2E7D52', cursor: 'pointer', fontWeight: 600 }}>
              Refresh
            </button>
          </div>
          ...
          {loading ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[1,2,3].map((i) => <div key={i} style={{ height: 96, background: '#F3F4F6', borderRadius: 12 }} />)}
            </div>
          ) : open.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 24px' }}>
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 12, color: '#D1D5DB' }}><InboxIcon className="w-12 h-12" /></div>
              <p style={{ fontWeight: 600, color: '#374151' }}>No open requests</p>
              <p style={{ fontSize: 13, color: '#6B8A7A', marginTop: 6 }}>New requests appear when consumers place orders</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 520, overflowY: 'auto' }}>
              {open.map((r) => (
                <div key={r.id} style={{ border: '1px solid rgba(46,125,82,0.1)', borderRadius: 14, padding: 16, transition: 'border-color 0.15s' }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                        <p style={{ fontSize: 14, fontWeight: 700, color: '#111827', textTransform: 'capitalize' }}>{r.crop_type}</p>
        ...
        {/* Active deliveries */}
        <div className="card" style={{ padding: 24 }}>
          <h2 style={{ fontWeight: 700, fontSize: 15, color: '#111827', marginBottom: 20 }}>My Active Deliveries</h2>

          {loading ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[1,2].map((i) => <div key={i} style={{ height: 112, background: '#F3F4F6', borderRadius: 12 }} />)}
            </div>
          ) : active.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 24px' }}>
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 12, color: '#D1D5DB' }}><TruckStatusIcon className="w-12 h-12" /></div>
              <p style={{ fontWeight: 600, color: '#374151' }}>No active deliveries</p>
              <p style={{ fontSize: 13, color: '#6B8A7A', marginTop: 6 }}>Accept a request to start</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 520, overflowY: 'auto' }}>
              {active.map((r) => (
                <div key={r.id} style={{ border: '1px solid rgba(46,125,82,0.12)', borderRadius: 14, padding: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                    <div>
                      <p style={{ fontSize: 14, fontWeight: 700, color: '#111827', textTransform: 'capitalize' }}>{r.crop_type}</p>
                      <p style={{ fontSize: 12, color: '#6B8A7A', marginTop: 3 }}>{r.quantity_kg} kg · {r.pickup_location}</p>
                    </div>
                    <StatusBadge status={r.status} />
                  </div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    {r.status === 'accepted' && (
                      <button
                        ...
                        style={{ flex: 1, padding: '9px 0', borderRadius: 9999, background: 'rgba(201,168,76,0.12)', border: '1px solid rgba(201,168,76,0.25)', color: '#C9A84C', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}
                      >
          ...
          {completed.length > 0 && (
            <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid rgba(46,125,82,0.08)' }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, color: '#374151', marginBottom: 10 }}>Completed ({completed.length})</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 160, overflowY: 'auto' }}>
                {completed.map((r) => (
                  <div key={r.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0' }}>
                    <div>
                      <p style={{ fontSize: 12, fontWeight: 600, color: '#374151', textTransform: 'capitalize' }}>{r.crop_type}</p>
                      <p style={{ fontSize: 11, color: '#6B8A7A' }}>{r.pickup_location} · {r.quantity_kg} kg</p>
                    </div>
                    <StatusBadge status={r.status} />
                  </div>
                ))}
