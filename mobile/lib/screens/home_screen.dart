              ...
              child: SlideTransition(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // ── ROLES — green band (white ink, white cards) ──
                    Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text('Marketplace',
              style: TextStyle(color: Color(0xFFA3C4B0), fontSize: 30, fontWeight: FontWeight.w800,
                  letterSpacing: -0.5)),
        ...
        children: items.asMap().entries.map((e) {
          ...
          return Expanded(
            child: Container(
              ...
              decoration: BoxDecoration(
                border: Border(right: isLast ? BorderSide.none : BorderSide(color: AppColors.border)),
              ),
              child: Column(children: [
                ...
                Text(e.value.$2, textAlign: TextAlign.center,
                    style: TextStyle(fontSize: 10, color: AppColors.textSecond, fontWeight: FontWeight.w500)),
              ]),
            ),
          );
        }).toList(),
...
class _SectionHeader extends StatelessWidget {
  ...
  final bool onBand; // rendered inside a green band → white ink
  const _SectionHeader({required this.tag, required this.title, this.onBand = false});
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(tag, style: TextStyle(
            color: onBand ? const Color(0xFFA3C4B0) : AppColors.brandMid,
            fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 1.2)),
        const SizedBox(height: 6),
        Text(title, style: TextStyle(
            fontSize: 24, fontWeight: FontWeight.w800,
            color: onBand ? Colors.white : AppColors.textPrimary,
...
class _RolesSection extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    // Cards float white on the green band in BOTH themes (same as the web
    // .section-brand treatment) → fixed light-mode ink values, never dynamic.
    const roles = [
      _RoleData('👨‍🌾', 'Farmer', AppColors.green, Color(0xFFDCE2DE),
          ['Post produce listings', 'Track orders in real time', 'View AI demand forecasts'], '/register'),
      _RoleData('🛒', 'Buyer', Color(0xFF48624F), Color(0xFFF3F5F3),
          ['Wholesaler, retailer or consumer', 'Browse produce by region', 'Track delivery status live'], '/register'),
      _RoleData('🚚', 'Transporter', Color(0xFF48624F), Color(0xFFDCE2DE),
...
class _RoleCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        // always white on the green band → fixed light-mode edge, not dynamic
        color: Colors.white,
      ...
      child: Column(
        ...
        children: [
          ...
          Text(data.role, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Colors.black)),
          ...
          ...data.points.map((p) => Padding(
            ...
            child: Row(children: [
              ...
              Expanded(child: Text(p, style: const TextStyle(fontSize: 11, color: Color(0x99000000)))),
            ]),
          )),
        ],
      ),
        ...
        children: items.map((item) => Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(item.$2, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
              const SizedBox(height: 6),
              Text(item.$3, style: TextStyle(fontSize: 11, color: AppColors.textSecond, height: 1.5)),
            ],
          ),
        )).toList(),
      ),
        ...
        Wrap(
          ...
          children: crops.map((c) => Container(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Text(c.trend,
                  style: TextStyle(
                      // direction is carried by the +/- sign; weight, not hue, separates them
                      color: c.positive ? Colors.white : Colors.white70,
